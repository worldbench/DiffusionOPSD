...
def get_config():
    ...
    # random seed. DiffusionOPSD policy: no fixed seed -> naturally random runs.
    # None means "do not seed"; a synchronized epoch nonce is drawn from os.urandom
    # on rank 0 and broadcast only to keep K-repeat prompt grouping aligned across
    # ranks (logged as run_random_nonce; NOT a reproducibility seed).
    config.seed = None
    ...
    ###### Prompt Function ######
    # Prompt reader used by the trainers for train.txt/test.txt manifests.
    config.prompt_fn = ""
