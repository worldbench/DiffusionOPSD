"""DiffusionOPD-Open3Teacher. See ``opd/README.md`` for the reproduction contract.

Student: 10-step CFG-free deterministic (dpm2) rollout. Distillation: over ALL 10 denoising steps,
minimize the MSE between student and each of the 3 DiffusionOPSD teachers on the flow-ODE TRANSITION
MEAN (solver prev_sample_mean = z_t + v*dt, i.e. dt^2-weighted velocity MSE — NOT raw velocity), at
each step's on-policy state, averaged 1:1:1 (full-trajectory transition distillation). The dpm2 rollout
only generates the on-policy states; the distilled transition is the flow field (velocity is the
fundamental, sampler-agnostic quantity — we do NOT match the dpm2 integrator's 2nd-order step). Reward
Reward models are absent from the second-stage objective and are used only for post-hoc evaluation.
#Iter = 300 optimizer updates; save every 10 updates by default.

Full-trajectory distill = heavier than DanceOPD (10 query steps vs 1). To bound memory we backward
per step (one velocity grad-graph alive at a time) and accumulate grads across the 10 steps + the
num_batches_per_epoch batches, then a single optimizer.step() per epoch.

Run:  torchrun --standalone --nproc_per_node=8 opd/train_diffusionopd_open3.py
Env same as DanceOPD (SAVE_DIR, LOG_DIR, OPD_TEACHER_LORAS, BENCHMARK=1, ...).
"""
import os
import sys
import json
...
from opd_common import (  # noqa: E402
    ...
    maybe_resume, BenchmarkTimer, flow_transition_mean,
)
...
def main():
    ...
    for epoch in range(first_epoch, n_updates):
        ...
        for i in range(config.sample.num_batches_per_epoch):
            ...
            # DiffusionOPD distills the teacher transition mean over all denoising steps.
            # At each on-policy state z_t, match the solver's prev_sample_mean (deterministic, eta=0) of
            # student vs each teacher — NOT the raw velocity. Since mean = z_t + v*dt, this is the
            # physically correct dt^2-weighted transition-mean MSE (raw velocity-MSE weights every step
            # equally, which collapses to a full-trajectory DanceOPD). Backward per step to bound memory.
            for step_idx in range(num_steps):
                ...
                with torch.cuda.amp.autocast(enabled=enable_amp, dtype=mp_dtype):
                    ...
                    mean_student = flow_transition_mean(z_t, v_student.float(), sigmas, step_idx, eta=0.0)
                    step_loss = sum(
                        w[j] * torch.nn.functional.mse_loss(
                            mean_student,
                            flow_transition_mean(z_t, v_teachers[j].float(), sigmas, step_idx, eta=0.0))
                        for j in range(len(v_teachers))
                    ) / (sum(w) * num_steps * grad_accum)
        ...
        if is_main_process(rank) and not bench_on:
            print(f"[{METHOD}] update {global_step} done", flush=True)
