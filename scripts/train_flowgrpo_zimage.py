...
from diffusionopsd.internvl_bridge import bridge_enabled as internvl_bridge_enabled  # INTERNVL_BRIDGE 2-GPU 26B reward server (gated; forward-only for flowgrpo)
...
def main(_):
    ...
    # dedicated 2-GPU-sharded reward server for flowgrpo too (the 26B co-located on the 6B Z-Image DiT
    # OOMs). flowgrpo uses the reward FORWARD-ONLY, so rewards.internvl_*_score ships the image to the
    # server (remote_reward_scores_forward) under INTERNVL_BRIDGE=1 -- no reward-gradient, no trainer
    # ref-plumbing (internvl_dual's ref is loaded in rewards.py from prompt_metadata['ref_path']).
    # Mutually exclusive with ZIMAGE_HEAVY_BRIDGE. ---
    ...
    FLOWGRPO_ZIMAGE_REWARDS = {"hpsv2", "pickscore", "clipscore", "aesthetic", "altclip",
                               "imagereward", "hpsv3", "deqa", "internvl_t2i", "internvl_dual"}
