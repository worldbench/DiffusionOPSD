#!/usr/bin/env python3
"""Rollout-budget-matched ReFL trainer for Z-Image-Turbo."""
...
from scripts import train_opsd_zimage as runtime
...
def _preencode_records(pipeline, records, rank: int, world_size: int, device, batch_size: int = 4):
    ...
    for start in range(0, len(local), batch_size):
        ...
        embeds = runtime.encode_prompts_list(pipeline, [x["prompt"] for x in chunk], device)
        for record, embedding in zip(chunk, embeds):
            encoded.append((record, embedding.detach().cpu()))
    return encoded


def _preencode_eval_batches(pipeline, records, rank: int, world_size: int, device):
    if not records:
        return []
    ...
    for batch_index, start in enumerate(range(0, len(records), canonical_batch)):
        ...
        if any(
            ...
            raise RuntimeError(f"canonical Z-Image eval batch metadata mismatch at batch {batch_index}")
        embeds = runtime.encode_prompts_list(pipeline, [record["prompt"] for record in chunk], device)
        encoded.append((chunk, [embedding.detach().cpu() for embedding in embeds]))
    return encoded


def _encode_update_cache(pipeline, local_micro_batches, device):
    unique: dict[int, str] = {}
    ...
    for start in range(0, len(indices), 4):
        ...
        embeds = runtime.encode_prompts_list(pipeline, [unique[index] for index in chunk], device)
        for index, embedding in zip(chunk, embeds):
            cache[index] = embedding.detach()
    return cache
...
def _late_x0(model, latents, sigmas, prompt_embeds, late_indices):
    z_t = torch.cat([latents[index][batch : batch + 1] for batch, index in enumerate(late_indices)]).detach().float()
    sigma = torch.stack([sigmas[index].detach().float() for index in late_indices]).to(z_t.device)
    velocity = runtime.zimage_v(model, z_t, sigma, prompt_embeds)
...
def _score_images_eval(scorer, reward: str, images: torch.Tensor, prompts, refs):
    ...
    with torch.no_grad():
        ...
        if reward in ZIMAGE_DIFF_BRIDGE_REWARDS and runtime.zimage_heavy_diff_bridge_enabled():
            from diffusionopsd.zimage_heavy_diff_bridge import remote_heavy_reward_forward
            return remote_heavy_reward_forward(images, prompts).float()
        if reward == "pickscore":
        ...
        if reward in {"clipscore", "hpsv2", "altclip"}:
...
def _forward_reward(pipeline, scorer, reward: str, x0: torch.Tensor, prompts, refs):
    with torch.no_grad():
        images = runtime.zimage_decode(pipeline.vae, x0)
        if reward == "internvl_t2i":
            from diffusionopsd.internvl_bridge import remote_reward_scores_forward
            return remote_reward_scores_forward(images, prompts).float()
        if reward == "internvl_dual":
...
def _evaluate(
    *, pipeline, model, scorer, reward, encoded_batches, manifest_records, config, device, group, rank,
    ref_root: str, output_path: str, update: int | str, score_scale: float,
):
    local_indices, local_scores = [], []
    model.eval()
    for records, embeddings_cpu in encoded_batches:
        embeddings = [embedding.to(device) for embedding in embeddings_cpu]
        generator = torch.Generator(device=device).manual_seed(int(records[0]["noise_seed"]))
        # Canonical Z-Image evaluation wraps the complete rollout and VAE decode
        # in bf16 autocast (native_eval.py and train_nft_zimage.py::eval_fn).
        # Omitting this changes the decoded pixels despite identical latents,
        # seeds, and generation batches, so keep this path bit-compatible.
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            out = runtime.zimage_rollout(
                pipeline, embeddings, num_inference_steps=9, height=config.resolution,
                width=config.resolution, device=device, generator=generator,
                guidance_scale=0.0, decode=True,
            )
        images = out["images"].detach().float()
    ...
    if rank == 0:
        ...
        for index, score in merged:
            ...
            per_image.append({
                "index": index, "prompt": source["prompt"], "ref_path": source.get("ref_path", ""),
                "seed": int(source["noise_seed"]), "score": score,
        ...
        payload = {
            ...
            "update": update,
            "protocol": {
                "sampler": "flow_euler", "num_steps": 9, "guidance_scale": 0.0,
...
def _calibrate(*, pipeline, model, scorer, reward, encoded_records, config, rr, device, group, rank):
    if not bool(rr.standardize_reward):
        payload = {
            "mode": "released_imagereward_raw_scale", "reward": reward,
            "raw_mean": None, "raw_std": None, "samples_global": 0,
            "hinge_margin": float(rr.hinge_margin), "used": False,
        }
        write_json(os.path.join(config.save_dir, "calibration.json"), payload, rank=rank)
        return 0.0, 1.0
    local_indices, local_scores = [], []
    model.eval()
    for record, embedding_cpu in encoded_records:
        embeddings = [embedding_cpu.to(device)]
        ...
        out = runtime.zimage_rollout(
            pipeline, embeddings, num_inference_steps=num_steps, height=config.resolution,
            width=config.resolution, device=device, generator=generator,
            guidance_scale=0.0, decode=False, stop_before_index=late_index,
        )
        if int(out["completed_prefix_steps"]) != late_index or len(out["latents"]) != late_index + 1:
            raise AssertionError("calibration must stop exactly before the sampled denoiser call")
        with torch.no_grad():
            x0, _sigma = _late_x0(model, out["latents"], out["sigmas"], embeddings, [late_index])
    ...
    if rank == 0:
        ...
        payload = {
            "mode": "frozen_base_random_late_one_step_x0", "reward": reward,
            "raw_mean": stats["mean"], "raw_std": stats["std"], "raw_se": stats["se"],
            "samples_global": stats["n"], "late_fraction": float(rr.late_fraction),
            "hinge_margin": float(rr.hinge_margin), "standardize_reward": True, "used": True,
        }
    else:
        payload = None
    objects = [payload]
    dist.broadcast_object_list(objects, src=0, group=group, device=device)
    payload = objects[0]
    write_json(os.path.join(config.save_dir, "calibration.json"), payload, rank=rank)
...
def main(_):
    ...
    diff_bridge = reward in ZIMAGE_DIFF_BRIDGE_REWARDS
    internvl_bridge = reward in INTERNVL_REWARDS
    if diff_bridge and internvl_bridge:
        raise RuntimeError("reward cannot require both bridge types")
    if diff_bridge:
        if not runtime.zimage_heavy_diff_bridge_enabled():
            raise RuntimeError(f"{reward} requires ZIMAGE_HEAVY_DIFF_BRIDGE=1")
        from diffusionopsd.zimage_heavy_diff_bridge import (
            HeavyDiffRewardServer, bridge_client_shutdown, bridge_server_devices,
            is_server_rank, make_bridge_groups, policy_group,
        )
        make_bridge_groups(launch_world)
        if is_server_rank(rank):
            HeavyDiffRewardServer(bridge_server_devices(local_rank)[0], reward_kind=reward).serve(
                n_policy=launch_world - 1
            )
            runtime.cleanup_distributed()
            return
        group = policy_group()
        policy_world = launch_world - 1
        shutdown_bridge = bridge_client_shutdown
    elif internvl_bridge:
        ...
        from diffusionopsd.internvl_bridge import (
            RewardServer, bridge_client_shutdown, bridge_server_devices, is_server_rank,
            make_bridge_groups, policy_group,
        )
        make_bridge_groups(launch_world)
        if is_server_rank(rank):
            devices = bridge_server_devices(local_rank)
            RewardServer(devices[0], devices, reward_kind=reward).serve(n_policy=launch_world - 1)
            runtime.cleanup_distributed()
            return
        group = policy_group()
        policy_world = launch_world - 1
    ...
    if launch_world != int(rr.expected_launch_world_size) or policy_world != int(rr.expected_policy_world_size):
        raise RuntimeError(
            f"topology mismatch: launch={launch_world}/{rr.expected_launch_world_size}, "
            f"policy={policy_world}/{rr.expected_policy_world_size}"
        )
    micro_batch = int(rr.micro_batch_size)
    if micro_batch != 1:
        raise RuntimeError("truncated-prefix ReFL currently requires micro_batch_size=1")
    accum = derive_gradient_accumulation_steps(int(rr.trajectories_per_update), policy_world, micro_batch)
    ...
    scaler = runtime.GradScaler(enabled=False)
    from diffusers import ZImagePipeline
    pipeline = ZImagePipeline.from_pretrained(config.pretrained.model, torch_dtype=dtype)
    ...
    rollout_transformer = transformer
    transformer.enable_gradient_checkpointing()
    pipeline.vae.enable_gradient_checkpointing()

    lora = LoraConfig(
        r=32, lora_alpha=64, init_lora_weights="gaussian", target_modules=runtime.ZIMAGE_LORA_TARGETS
    )
    ...
    # PEFT injects LoRA layers into the raw S3-DiT in place.  Keep the pipeline
    # pointed at that raw module, exactly like train_nft_zimage.py and
    # native_eval.py --lora; only the differentiable late-state rerun goes
    # through the DDP/PeftModel wrapper.  Replacing pipeline.transformer with
    # the wrapper changes the Z-Image raw rollout numerics even for zero LoRA.
    if pipeline.transformer is not rollout_transformer:
        raise RuntimeError("PEFT unexpectedly replaced the pipeline's raw Z-Image transformer")
    ...
    calibration_records = fixed_manifest_records(
        load_prompt_records(str(rr.calibration_manifest_path)), count=int(rr.calibration_num_prompts),
        noise_seed=int(rr.calibration_noise_seed), late_seed=int(rr.calibration_late_seed),
    )
    final_records = fixed_eval_manifest_records(
        load_prompt_records(str(rr.final_manifest_path)), count=int(rr.final_eval_num_prompts),
    ...
    encoded_final = _preencode_eval_batches(pipeline, final_records, rank, policy_world, device)
    encoded_curve = _preencode_eval_batches(pipeline, curve_records, rank, policy_world, device)

    scorer = None if (diff_bridge or internvl_bridge) else runtime._load_reward_scorer(reward, device)
    ...
    if resume_checkpoint:
        ...
        objects = [calibration_payload]
        dist.broadcast_object_list(objects, src=0, group=group, device=device)
        calibration_payload = objects[0]
        if reward == "imagereward":
        ...
        calibration_mean, calibration_std = _calibrate(
            ...
            encoded_records=encoded_calibration, config=config, rr=rr, device=device,
    ...
    if not resume_checkpoint:
        ...
        try:
            eval0 = _evaluate(
                pipeline=pipeline, model=model.module, scorer=scorer, reward=reward,
                encoded_batches=encoded_curve, manifest_records=curve_records, config=config, device=device,
                group=group, rank=rank, ref_root=str(rr.pairwise_eval_ref_root),
                output_path=os.path.join(config.save_dir, "curve", "update_0000.json"), update=0,
    ...
    run_config = {
        "backbone": "Z-Image-Turbo", "method": "rollout-budget-matched ReFL",
        "reward": reward,
        ...
        "algorithm": {
            ...
            "current_policy_fresh_noise_rollout": True, "prefix_no_grad": True,
            ...
            "late_state_per_trajectory": 1, "late_fraction": float(rr.late_fraction),
            "endpoint_regression": False, "group_advantage": False,
            "diffusion_pretraining_loss": False, "reference_mse_or_kl": False,
        ...
        "batch": {
            ...
            "micro_batch_size": micro_batch, "policy_world_size": policy_world,
    ...
    retained_objects = [retained if rank == 0 else None]
    dist.broadcast_object_list(retained_objects, src=0, group=group, device=device)
    cumulative_train_seconds = float(retained_objects[0][0])
    cumulative_online_eval_seconds = float(retained_objects[0][1])
    replay_boundary_online_eval = bool(retained_objects[0][2])
    ...
    if replay_boundary_online_eval:
        ...
        try:
            result = _evaluate(
                ...
                config=config, device=device, group=group, rank=rank,
    ...
    while global_step < int(rr.num_updates):
        ...
        update_records = build_update_records(
            train_records, update_index=global_step,
            distinct_prompt_groups=int(rr.distinct_prompt_groups),
            trajectories_per_prompt=int(rr.trajectories_per_prompt),
            prompt_seed=int(rr.train_prompt_seed), shuffle_seed=int(rr.train_shuffle_seed),
            noise_seed=int(rr.train_noise_seed), late_seed=int(rr.train_late_seed),
        )
        ...
        embed_cache = _encode_update_cache(pipeline, local_batches, device)
        ...
        for micro_index, records in enumerate(local_batches):
            ...
            embeddings = [embed_cache[int(record["dataset_index"])] for record in records]
            generators = [
            ...
            out = runtime.zimage_rollout(
                pipeline, embeddings, num_inference_steps=num_steps, height=config.resolution,
                width=config.resolution, device=device, generator=generators,
                guidance_scale=0.0, decode=False, stop_before_index=late_indices[0],
            )
            if (
                int(out["completed_prefix_steps"]) != late_indices[0]
                or len(out["latents"]) != late_indices[0] + 1
            ):
                raise AssertionError("training rollout executed beyond the sampled late state")
            model.train()
            with maybe_no_sync(model, micro_index == len(local_batches) - 1):
                x0, sigma = _late_x0(model, out["latents"], out["sigmas"], embeddings, late_indices)
        ...
        record = {
            ...
            **_flatten_metrics("late_timestep_index", late_stats),
            "loss": loss_stats["mean"], "grad_norm": float(grad_sync),
            ...
            "skipped_suffix_forwards_mean": 9 - late_stats["mean"] - 1.0,
    ...
    if rank == 0 and final_result is not None:
        write_json(os.path.join(config.save_dir, "final_eval", "per_image.json"), final_result["per_image"], rank=rank)
    final_eval_seconds = time.perf_counter() - final_eval_started
    write_json(
        os.path.join(config.save_dir, "run_done.json"),
        {
            "status": "PASS_PENDING_EXTERNAL_VALIDATOR", "global_step": global_step,
            "cumulative_trajectories": cumulative_trajectories,
