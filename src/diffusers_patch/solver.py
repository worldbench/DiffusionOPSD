...
def run_sampling(
    ...
    full_steps = len(sigma_schedule) - 1
    steps_to_run = full_steps if max_steps is None else int(max_steps)
    if not 0 <= steps_to_run <= full_steps:
        raise ValueError(f"max_steps must be in [0,{full_steps}], got {steps_to_run}")
    for i in tqdm(
        range(steps_to_run),
