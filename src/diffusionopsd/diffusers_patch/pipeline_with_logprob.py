...
def pipeline_with_logprob(
    ...
    # 7. Denoising loop
    if stop_before_index is not None and not 0 <= int(stop_before_index) < len(timesteps):
        raise ValueError(
            f"stop_before_index must select a denoiser call in [0,{len(timesteps) - 1}], "
    ...
    latents, all_latents, all_log_probs = run_sampling(
        ...
        deterministic,
        noise_level,
        max_steps=stop_before_index,
    )

    # ReFL samples the training timestep before denoising and only needs the
    # state immediately before that call.  Returning here avoids both the
    # unused suffix forwards and the unused endpoint VAE decode.  Passing the
    # full sigma schedule into run_sampling preserves DPM solver prefix state;
    # truncating the schedule itself would incorrectly trigger its final-step
    # branch early.
    if not decode:
        if flux:
            return None, all_latents, latent_image_ids, text_ids, all_log_probs
        return None, all_latents, all_log_probs
